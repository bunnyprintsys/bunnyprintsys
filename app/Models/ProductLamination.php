...
class ProductLamination extends Model
{
    ...
    public function scopeLaminationId($query, $value)
    {
        $columnName = $this->getAliasColumnName('lamination_id');
        if (is_array($value)) {
            return $query->whereIn($columnName, $value);
        }
        return $query->where($columnName, $value);
    ...
    public function scopeFilter($query, $input, $alias = null, $like = true)
    {
        ...
        if (Arr::get($input, 'lamination_id', false)) {
    ...
    public function scopeSortBy($query, $input, $alias = null)
    {
        ...
        $sortable = ['id', 'name', 'laminations.name'];
